...
newperms() { # Set special sudoers settings for install (or after).
	sed -i "/#observer/d" /etc/sudoers
	echo "$* #observer" >> /etc/sudoers ;}
...
manualinstall() { # Installs $1 manually if not installed. Used only for AUR helper here.
	...
	cd /tmp || exit 1
	rm -rf /tmp/"$1"*
	curl -sO https://aur.archlinux.org/yay.git/snapshot/"$1".tar.gz &&
...
maininstall() { # Installs all needed programs from main repo.
	dialog --title "Observer Installation" --infobox "Installing \`$1\` ($n of $total). $1 $2" 5 70
	installpkg "$1"
	}
...
gitmakeinstall() {
	...
	dialog --title "Observer Installation" --infobox "Installing \`$progname\` ($n of $total) via \`git\` and \`make\`. $(basename "$1") $2" 5 70
...
aurinstall() { \
	dialog --title "Observer Installation" --infobox "Installing \`$1\` ($n of $total) from the AUR. $1 $2" 5 70
	echo "$aurinstalled" | grep -q "^$1$" && return 1
	sudo -u "$name" $aurhelper -S --noconfirm "$1" >/dev/null 2>&1
	}

pipinstall() { \
	dialog --title "Observer Installation" --infobox "Installing the Python package \`$1\` ($n of $total). $1 $2" 5 70
...
for x in curl base-devel git ntp zsh; do
	dialog --title "Observer Installation" --infobox "Installing \`$x\` which is required to install and configure other programs." 5 70
	installpkg "$x"
done

dialog --title "Observer Installation" --infobox "Synchronizing system time to ensure successful and secure installation of software..." 4 70
...
newperms "%wheel ALL=(ALL) ALL #observer
